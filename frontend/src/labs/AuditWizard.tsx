...
export const AuditWizard: React.FC = () => {
  ...
  const runAudit = async () => {
    ...
    setNextAction(null);
    try {
      const resp = await fetch('http://localhost:8000/labs/audit/run', {
        method: 'POST',
  ...
  if (step === 4 && result) {
    return (
      <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        <p className="text-sm text-slate-400 mb-4">Your diagnostic</p>
            ...
            <button
              ...
              onClick={() => {
                try {
                  ...
                  console.warn('Failed to send feedback for audit next action', e);
                }

                if (nextAction.type === 'open_lab_tool') {
                  const lab = (nextAction.payload && (nextAction.payload.lab_tool || nextAction.payload.lab)) || nextAction.target || 'build_estimator';
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500 mb-1">Product</p>
            <p className="text-2xl font-semibold">{result.scores.product}/100</p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500 mb-1">Engineering</p>
            <p className="text-2xl font-semibold">{result.scores.engineering}/100</p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500 mb-1">Data &amp; AI</p>
            <p className="text-2xl font-semibold">{result.scores.data_ai}/100</p>
          </div>
        </div>
  ...
  return (
    <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
      ...
      {step === 0 && (
        <div>
          <h2 className="text-lg font-semibold mb-2">Where is your product today?</h2>
          <div className="space-y-4">
            <div>
              <p className="text-sm text-slate-300 mb-2">Product stage</p>
              <div className="grid grid-cols-2 gap-2 text-sm">
                {[
                  ...
                  <button
                    key={val}
                    onClick={() => setForm((f) => ({ ...f, product_stage: val }))}
            ...
            <div>
              <p className="text-sm text-slate-300 mb-2">Release cadence</p>
              <div className="grid grid-cols-2 gap-2 text-sm">
                {[
                  ['ad_hoc', 'Ad-hoc'],
                  ['monthly', 'Monthly'],
                  ...
                  <button
                    key={val}
                    onClick={() => setForm((f) => ({ ...f, release_freq: val }))}
                    className={`rounded-lg border px-3 py-2 text-left ${
      ...
      {step === 1 && (
        <div>
          <h2 className="text-lg font-semibold mb-2">Engineering foundations</h2>
          <div className="space-y-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm text-slate-300">CI/CD pipeline in place?</p>
                <p className="text-xs text-slate-500">
                  E.g. GitHub Actions, GitLab CI, Azure DevOps, etc.
                </p>
              </div>
              <button
                onClick={() => setForm((f) => ({ ...f, ci_cd: !f.ci_cd }))}
                className={`inline-flex items-center rounded-full border px-3 py-1 text-xs ${
                  form.ci_cd ? 'border-emerald-400 bg-emerald-500/10' : 'border-slate-600'
                }`}
              >
                {form.ci_cd ? 'Yes' : 'No'}
              </button>
            ...
            <div>
              <p className="text-sm text-slate-300 mb-2">Automated testing maturity</p>
              <div className="grid grid-cols-2 gap-2 text-sm">
                {[
                  ['none', 'No tests'],
                  ...
                  <button
                    key={val}
                    onClick={() => setForm((f) => ({ ...f, testing: val }))}
                    className={`rounded-lg border px-3 py-2 text-left ${
                      form.testing === val
                        ? 'border-blue-500 bg-blue-600/20'
                        : 'border-slate-700 hover:border-slate-500'
      ...
      {step === 2 && (
        <div>
          <h2 className="text-lg font-semibold mb-2">Data &amp; analytics baseline</h2>
          <div className="space-y-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm text-slate-300">Data is centralised?</p>
                <p className="text-xs text-slate-500">
                  Warehouse / lake or at least one &quot;source of truth&quot;.
                </p>
              </div>
              <button
                onClick={() => setForm((f) => ({ ...f, data_centralized: !f.data_centralized }))}
                className={`inline-flex items-center rounded-full border px-3 py-1 text-xs ${
                  form.data_centralized ? 'border-emerald-400 bg-emerald-500/10' : 'border-slate-600'
                }`}
              >
            ...
            <div>
              <p className="text-sm text-slate-300 mb-2">Analytics maturity</p>
              <div className="grid grid-cols-2 gap-2 text-sm">
                {[
                  ...
                  <button
                    key={val}
                    onClick={() => setForm((f) => ({ ...f, analytics: val }))}
                    className={`rounded-lg border px-3 py-2 text-left ${
                      form.analytics === val
                        ? 'border-blue-500 bg-blue-600/20'
                        : 'border-slate-700 hover:border-slate-500'
      ...
      {step === 3 && (
        <div>
          <h2 className="text-lg font-semibold mb-2">Where does it hurt the most?</h2>
          <p className="text-sm text-slate-300 mb-3">
            This helps us shape the recommendations. Pick all that apply.
          </p>
          <div className="grid grid-cols-2 gap-2 text-sm">
            {[
              'slow_releases',
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        {step === 3 && (
          <button
            onClick={runAudit}
            disabled={loading}
            className="inline-flex items-center rounded-lg bg-emerald-500 px-4 py-2 text-sm font-medium hover:bg-emerald-400 disabled:opacity-60"
          >
